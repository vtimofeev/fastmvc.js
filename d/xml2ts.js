...
(function (xml2ns) {
    ...
    var ALLOWED_KEYS = [].concat(KEYS, EVENT_KEYS);
    var Xml2Ts = (function () {
        ...
        Xml2Ts.prototype.parse = function (fileName, content, next) {
            ...
            async.series([
                ...
                dust.render('object.ts.dust', rootDom, function (e, result) {
                    ...
                    var filePath = __dirname + t.srcOut + '/' + rootDom.className + '.ts';
    ...
    var Xml2TsUtils = (function () {
        ...
        Xml2TsUtils.getDynamicValues = function (key, value, delimiter) {
            ...
            if (dynamicValues)
                return { static: staticValues, dynamic: dynamicValues, value: value };
            return value;
        };
        ...
        Xml2TsUtils.extendDynamicSummary = function (dynamic, dynamicPropertyName, domPropertyName, elementPath, values) {
            if (!dynamic[dynamicPropertyName])
                dynamic[dynamicPropertyName] = {};
            if (!dynamic[dynamicPropertyName][domPropertyName])
                dynamic[domPropertyName][domPropertyName] = {};
            if (!dynamic[dynamicPropertyName][domPropertyName][elementPath])
                dynamic[domPropertyName][domPropertyName][elementPath] = (values.length === 1) ? values[0] : values;
        };
        Xml2TsUtils.recreateJsNode = function (data, path, rootObject) {
            var a = data.attribs;
            var object = { path: path, type: data.type, staticAttributes: [], children: [] };
            if (!rootObject) {
                rootObject = object;
