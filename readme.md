...
В более сложном варианте - модели могут быть производными от одной или нескольких исходных моделей, выполнять с ними функции объединения(пересечения,разницы), 
и получая результат применять к данным различные функторы (filter,map,reduce).
При изменении исходных моделей и автоматически обновляются и отправляют событие изменения.

*Mediator* - создает и связывает представления с соотвествующими моделями и фасадом.
Также предоставляет доступ представлений к глобальному вью.
